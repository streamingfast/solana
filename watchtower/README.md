...
Watchtower also alerts when a monitored validator's vote account balance is
below the bank-side VAT balance threshold computed from RPC-visible state. This
check only verifies the vote account balance over public RPC. It does not verify
full VAT eligibility, including BLS pubkey presence, stake, top validator set
inclusion, or tie cutoff behavior.
